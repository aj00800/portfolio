...
export default function IMessageWidget() {
  ...
  const finishConversation = () => {
    ...
    try {
      ...
      const mailtoHref = `mailto:aj08abdullah@gmail.com?subject=${encodeURIComponent(
        subject
  ...
  return (
    <div
      className="relative overflow-hidden rounded-3xl border border-gray-200 bg-white/90 p-6 text-black shadow-xl backdrop-blur sm:p-8"
    ...
    >
      <div className="flex h-[360px] flex-col justify-between gap-4">
        <div
          ref={messagesRef}
          onWheel={handleWheel}
          className="scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent space-y-4 overflow-y-auto scroll-smooth pr-1"
        >
              ...
              >
                {message.side === 'incoming' ? (
                  <div className="flex max-w-[82%] items-start gap-3">
                    <div className="mt-4 h-12 w-12 overflow-hidden rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white font-semibold text-sm">
                      AJ
                    </div>
                    <div className="space-y-1">
                      <span className="block text-xs font-medium text-gray-500 sm:text-[13px]">
                        AJ
                      </span>
                      <div className="rounded-2xl rounded-tl-md bg-gray-100 px-4 py-2 text-[16px] leading-relaxed text-black shadow-sm sm:text-[17px]">
                        {message.text}
                      </div>
                    </div>
                  </div>
          ...
          {isTyping && (
            ...
            >
              <div className="flex items-center gap-2 rounded-2xl rounded-tl-md bg-gray-200 px-4 py-2 shadow-sm">
                {[0, 1, 2].map((dot) => (
                  <motion.span
                    key={dot}
                    custom={dot}
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 rounded-full bg-gray-100 px-3 py-2">
            <input
              ...
              className="text-md flex-1 border-none bg-transparent text-black placeholder:text-gray-500 focus:outline-none focus:ring-0 sm:text-sm"
            />
            <button
              type="button"
              onClick={handleSend}
